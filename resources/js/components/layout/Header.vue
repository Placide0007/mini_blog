<template>
  <header
    class="navbar navbar-expand-md bg-white border-bottom shadow-sm navbar-white px-5 py-0 sticky-top"
  >
    ...
    <div class="collapse navbar-collapse" id="links">
      <ul class="navbar-nav ms-auto me-auto">
        ...
        <router-link
          ...
          active-class="active"
          to="/postcreate"
          >Creer un poste</router-link
        >
      </ul>
      <div v-if="conn" class="btn-group">
        <button class="btn mb-3 mb-md-0 btn-dark">{{ userData.pseudo }}</button>
        <router-link class="btn btn-primary mb-3 mb-md-0" @click="logout" to="/register"
          >Se deconnecter</router-link
        >
      </div>
      <div v-else>
        <router-link class="btn btn-primary mb-3 mb-md-0" to="/login"
